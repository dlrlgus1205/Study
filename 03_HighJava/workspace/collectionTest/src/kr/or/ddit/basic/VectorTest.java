...
public class VectorTest {
	public static void main(String[] args) {
		// Vector 는 자바의 초기부터 지원하는 객체로 동기화 처리가 돼있다.
		...
		//		==> '삭제할데이터'를 찾아서 삭제한다.
		//		==> '삭제할데이터'가 여러개이면 이 중에 제일 첫 번째로 검색된 자료가 삭제된다.
		 ...
		 * 							   외부에서 지정해주는 기법으로 객체를 선언할 때 < >괄호 안에 그 객체의 
		 ...
		 *   대체해서 사용해야 한다.
		 * - 제네릭 타입으로 선언하게 되면 데이터를 꺼애올 때 별도의 형변환이 필요 없다.
		 */
		
		Vector<Integer> v2 = new Vector<Integer>();	// int형 자료만 저장할 수 있는 벡터
		Vector<String> v3 = new Vector<String>();	// String형 자료만 저장할 수 있는 벡터
		
		v3.add("안녕하세요");
		// v3.add(100);	// 오류 : 다른 종류의 데이터를 저장할 수 없다.
		
		String sTemp2 = v3.get(0);	// String형 자료에만 저장한 곳에서 꺼내기 때문에 형변환을 해줄 필요성이 x
		...
		v3.clear();
		System.out.println("v3의 크기 => " + v3.size());
