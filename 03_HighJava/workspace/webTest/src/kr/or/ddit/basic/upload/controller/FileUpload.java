...
/*
 * - Servlet 3.0이상에서 파일 업로드를 하려면 서블릿에 @MultipartConfig 애노테이션을을 설정해야 한다.
 * 
...
public class FileUpload extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// upload한 파일이 여러 개일 경우에 파일 목록이 저장될 List 객체 생성
		List<FileInfoVo> fileList = new ArrayList<FileInfoVo>();
		
		/*
			- 서블릿 3.0 이상에서 새롭게 추가된 Upload용 메서드
			  1) Request객체.getParts() ==> 전체 Part 객체를 Collection 객체에 담아서 반환한다.
			  2) Request객체.getPart("이름") ==> <form> 태그 안에 입력요소의 name 속성 값으로 구별한다.  
		*/
		
		// 전체 Part 객체 개수만큼 반복 처리
		for(Part part : request.getParts()) {
			String fileName = extractFileName(part); // upload한 파일 이름 구하기
			
			// 찾은 파일 이름이 null이면 파일이 아닌 일반 파라미터 데이터가 된다.
			if(fileName != null) { // 파일인지 검사
				// 1개의 파일 정보가 저장될 FileInfoVo 객체 생성
				FileInfoVo fvo = new FileInfoVo();
				
				fvo.setFile_writer(userName); // 작성자를 Vo 객체에 저장
				fvo.setOrigin_file_name(fileName); // 원래의 파일명을 Vo 객체에 저장
				
				// 실제 저장되는 파일 이름이 중복되는 것을 방지하기 위해서 UUID 객체를 이용하여 저장할 파일명을 만든다.
				String saveFileName = UUID.randomUUID().toString() + "_" + fileName;
				
				// 새로 만든 파일명을 Vo 객체에 '저장파일명'으로 저장
				fvo.setSave_file_name(saveFileName);
				
				// Part객체.getSize() ==> Upload된 파일의 크기 반환(단위 : byte)
				
				// byte 단위의 파일 크기를 KB 단위로 변환해서 Vo에 저장
