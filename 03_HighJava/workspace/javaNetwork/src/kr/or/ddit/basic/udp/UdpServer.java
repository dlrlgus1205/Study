 ...
 * 	 UDP 방식은 Datagram을 이용해서 송수신한다.
 */
public class UdpServer {
	public static void main(String[] args) {
		try {
			// DatagramSocket 객체 생성 ==> 포트 번호를 지정해서 생성한다.
			...
			while(true) {
				...
				// 데이터 수신하기 ==> Socket 객체의 receive() 메서드 이용
				// - receive() 메서드는 데이터가 올 때까지 기다린다.
				// - 수신된 데이터의 패킷 정보는 인수로 지정한 패킷변수(inPacket)에 저장된다.
				socket.receive(inPacket);
				
				// 받은 패킷에서 상대방의 IP 정보, 포트번호 등을 알 수 있다.
...
//				String msg = new String(inMsg, 0, inPacket.getLength(), "utf-8");
				...
				// 송신용 패킷객체를 생성한다.
				// 		==> 송신할 데이터가 저장된 byte형 배열, 송신할 자료의 길이(배열의 길이), 상대방 주소 정보, 상대방 포트 번호 이렇게 4가지를 인수로 지정하여 생성한다.
				DatagramPacket outPacket = new DatagramPacket(sendMsg, sendMsg.length, address, port);
				
				// 메시지 전송하기 ==> Socket 객체의 send() 메서드를 이용한다.
