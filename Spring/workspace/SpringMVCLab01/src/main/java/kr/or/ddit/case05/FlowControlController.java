...
public class FlowControlController {
	...
	public String handler5(RedirectAttributes redirectAttributes) {
		...
		redirectAttributes.addFlashAttribute("info", "start3 에서 만든 모델");
		redirectAttributes.addFlashAttribute("info1", "start3 에서 만든 모델1");
		redirectAttributes.addFlashAttribute("info2", "start3 에서 만든 모델2");
		return "redirect:/case05/dest3";
	}
	@RequestMapping("dest3")
	public void handler6(Model model) {
		log.info("dest3 번 요청 접수");
	...
	public String handler3(RedirectAttributes redirectAttributes) {
		...
		redirectAttributes.addFlashAttribute("info", "start2 에서 만든 모델");
		return "redirect:/case05/dest2";
	}
	@RequestMapping("dest2")
	public void handler4(Model model) {
		log.info("dest2 번 요청 접수 , model : {}", model.getAttribute("info"));
	}
	...
	public void handler2(@RequestAttribute(required = false) String info, Model model) {
