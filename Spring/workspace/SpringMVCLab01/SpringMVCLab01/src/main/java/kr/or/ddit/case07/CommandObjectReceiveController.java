...
public class CommandObjectReceiveController {
	...
	public String formHandler2( @ModelAttribute("adrs") AddressBookVO adrs ) {
		return "case07/formView2";
	}
	
	@PostMapping("commandObject2")
	public String processHandler2(@Validated @ModelAttribute("adrs") AddressBookVO adrs, Errors errors) {
		if(!errors.hasErrors()) {
	...
	public String formHandler3(Model model) {
		if(!model.containsAttribute("adrs")) {
			...
			log.info("검증 실패후 리다이렉션 : model : {}, errors : {}", 
					model.getAttribute("adrs"), model.getAttribute(BindingResult.MODEL_KEY_PREFIX+"adrs"));
		}
		return "case07/formView2";
	}
	
	@PostMapping("commandObject3")
	public String processHandler3(@Validated @ModelAttribute("adrs") AddressBookVO adrs, Errors errors
