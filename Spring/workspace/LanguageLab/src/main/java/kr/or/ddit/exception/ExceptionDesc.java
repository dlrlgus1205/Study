 ...
 * Throwable -- Error : 런타임에 직접 처리가 불가능한 시스템 폴트 상황등의 심각 정도가 다른 Throwable 객체
 * 			-- Exception : 필요에 의해 예외 처리 정책에 따라 직접 처리(회피)가 가능한 Throwable 객체
 * 				- Checked Exception : 예외가 발생 가능한 로직에서 발생 가능한 예외를 반드시 직접 처리해야하는 예외
 * 					ex) IOException, SQLException
 * 				- UnChecked Exception (RuntimeException 하위) : 직접 예외를 처리하지 않더라도 호출 구조를 통해 최종 JVM 까지 던져지는 예외 (모든 메소드의 선언부에 throws RuntimeException 구문이 묵시적으로 포함돼있음)
 * 					ex) NullPointerException, IllegalAccessException, NumberFormatException
 *
 *	** 예외 처리 정책
 *	1. 적극적인 처리(전환, 복구)
 *		try(){
 ...
 *		}
 *	2. 소극적인 처리(회피) : throws 키워드로 호출자에게 에외 처리를 떠넘기는 구조
 *
 *	*** 커스텀 예외 정의 장법
