...
// 	}
	Locale locale = Optional.ofNullable(localeP).filter(lp->lp.isEmpty()).map(lp->Locale.forLanguageTag(lp)).orElse(request.getLocale());
	
	ZoneId zone = Optional.ofNullable(zoneP).filter(zp->!zp.isEmpty()).map(zp->ZoneId.of(zp)).orElse(ZoneId.systemDefault());
...
// 		thisMonth = YearMonth.of(Integer.parseInt(year), Integer.parseInt(month));
// 	}
%>
<!DOCTYPE html>
<html>
...
<form id = "calForm">
	...
	<select name = "month">
		...
		%>
		<option value="<%=single.getValue() %>"><%=single.getDisplayName(TextStyle.FULL, locale) %></option>
		<%
			}
		%>
	</select>
