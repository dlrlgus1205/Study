...
//	3. Strategy pattern : 전략을 주입받아 사용하는 구조. 전략의 주입자(모든 결합력 집중)와 주입 방법이 필수 ===> DI container
...
//	@Autowired
	...
	public SampleServiceImpl(SampleDAO dao) {
		super();
		this.dao = dao;
		log.info("{} 객체 생성 및 {} 를 생성자로 주입 받음", this.getClass().getSimpleName(), dao.getClass().getSimpleName());
	}
  
	public void setDao(SampleDAO dao) {
		this.dao = dao;
		log.info("{} 를 기본 생성자로 생성, {} 를 setter 로 주입 받음", this.getClass().getSimpleName(), dao.getClass().getSimpleName());
	}
	
	public void init() {
		log.info("--------------- 주입된 dao : {}-------------------> {} init 호출, 언제 호출되는지 확인할 것", dao.getClass().getSimpleName(), this.getClass().getSimpleName());
	}
