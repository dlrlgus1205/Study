 ...
 * 3. entry point에서 컨테이너 객체 생성
 * 		ApplicationContext -> ConfigurableApplicationContext -> 컨테이너의 특성에 따라 구현체 결정
 * 4. 컨테이너로부터 필요한 객체 주입 : getBean(bean 검색조건 - class, id)
 * 
 * 스프링 컨테이너의 빈 관리 정책
 * 1. singleton : 등록된 빈을 대상으로 싱글턴 인스턴스를 운영함. scoope 속성으로 변경 가능
 * 		- singleton : 기본 값(한 개의 빈으로 하나의 인스턴스를 운영)
 * 		- prototype : 주입 시마다 새로운 인스턴스를 생성하고 운영
 * 		- request / session : 웹 어플리케이션에서 한정적으로 사용하는 값으로
 * 								인스턴스를 request 나 session 단위로 생성하고 운영
 * 2. 컨테이너가 초기화될 때 등록된 모든 빈의 인스턴스를 일시에 생성함. lazy-init
 * 		- true : 주입 시점까지 객체의 생성이 미뤄짐
 * 		- false : 컨테이너가 초기화될 때 미리 생성함
...
public class Case3Playground {
	public static void main(String[] args) throws InterruptedException {
		ConfigurableApplicationContext context = new GenericXmlApplicationContext("/kr/or/ddit/case3/conf/Case3-Context.xml");
...
//		
//		log.info("t003 : {}", t003);
//		log.info("service1 과 service2 동일 객체? : {}", service1 == service2);
//		log.info("service2 와 service3 동일 객체? : {}", service2 == service3);
//		
////		context.close();
